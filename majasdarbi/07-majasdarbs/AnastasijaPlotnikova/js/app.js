...
let renderTasks = () => {
    ...
    taskList.forEach((item, inx) => {
        let task = `
        <li class="list-group-item" >
            <div class="${item.done ? 'done' : ''}" data-index='${inx}' onclick="toggleDone(event)">
                ${item.textTask}
                <span class="remove" data-index='${inx}' onclick="removeTask(event)">❌</span> 
            </div> 
        </li>`;
